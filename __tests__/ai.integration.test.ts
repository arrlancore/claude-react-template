  ...
  GoogleGenerativeAI: jest.fn().mockImplementation(() => ({
    getGenerativeModel: jest.fn().mockReturnValue({
      generateContent: jest.fn().mockResolvedValue({
        response: {
          text: () =>
            "Mocked AI response: The two-pointer technique uses two pointers to efficiently solve array problems.",
        },
      }),
...
describe("AI System Integration Tests", () => {
  ...
  describe("GeminiClient", () => {
    ...
    test("should handle different models", async () => {
      const proResponse = await client.generateResponse("Test", {
        model: "pro",
      });
      const flashResponse = await client.generateResponse("Test", {
        model: "flash",
      });
  ...
  describe("AdaptiveEngine", () => {
    ...
    test("should assess user successfully", async () => {
      ...
      expect(["beginner", "intermediate", "advanced"]).toContain(
        assessment.user_level
      );
      expect(["fast", "balanced", "thorough"]).toContain(
        assessment.learning_pace
      );
    ...
    test("should provide guidance successfully", async () => {
      ...
      expect([
        ...
        "correction",
        "next_step",
      ]).toContain(guidance.response_type);
    ...
    test("should validate solution successfully", async () => {
      ...
      expect(["surface", "functional", "deep"]).toContain(
        validation.understanding_level
      );
    ...
    test("should handle chat successfully", async () => {
      ...
      expect(["answer", "question", "guidance", "encouragement"]).toContain(
  ...
  describe("AIUtils", () => {
    ...
    test("should validate response quality", () => {
      const goodResponse =
        "This is a comprehensive response that provides valuable information to the user.";
      const badResponse = "";
